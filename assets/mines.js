...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-3205ce2c";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-3205ce2c">'+'<span class="tile-face-3205ce2c tile-front-3205ce2c"></span>'+'<span class="tile-face-3205ce2c tile-back-3205ce2c"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-3205ce2c")){return;}
var back=tile.querySelector(".tile-back-3205ce2c");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-3205ce2c");if(isMine){tile.classList.add("boom-3205ce2c");}
if(dim){tile.classList.add("dimmed-3205ce2c");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-3205ce2c")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-3205ce2c");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-3205ce2c")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-3205ce2c");void panelEl.offsetWidth;panelEl.classList.add("jolt-3205ce2c");}
endRound();}
